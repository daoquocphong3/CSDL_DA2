...
END 
GO
--10 xuất ra danh mục sản phẩm với số lượng sản phẩm thuộc từng danh mục đó (năm 2020)
SELECT A.MALOAI,A.TENLOAI,COUNT(*) AS SOLUONG
FROM LOAISP A LEFT JOIN SANPHAM B ON A.MALOAI=B.MALOAI 
GROUP BY A.TENLOAI,A.MALOAI

GO
--11 Thống kế số lượng sản phẩm bán ra theo từng năm
SELECT D.MASP,D.SOLUONG,E.TENSP,E.IMAGEURL
FROM (SELECT TOP(20) B.MASP,SUM(B.SOLUONG) AS SOLUONG
        FROM DONHANG A JOIN CT_DONHANG B ON A.MADH=B.MADH 
        WHERE YEAR(A.NGAYLAP)=2020
        GROUP BY MASP) AS D JOIN SANPHAM E ON D.MASP=E.MASP
ORDER BY D.SOLUONG DESC
GO
-- Thống kế doanh phần trăm bán chạy sản phẩm nhất của các doanh mục (năm 2020)
SELECT D.MALOAI,D.SOLUONG,E.TENLOAI
FROM (SELECT B.MALOAI,SUM(A.SOLUONG) AS SOLUONG 
        ...
        WHERE YEAR(F.NGAYLAP)=2020 
        GROUP BY B.MALOAI) 
AS D JOIN LOAISP E ON D.MALOAI=E.MALOAI

GO
--12 thông kê doanh thu theo từng năm (2020)
SELECT NGAYLAP,TONGTIEN FROM DONHANG WHERE YEAR(NGAYLAP)=2020
GO
--DROP PROC MIN_SP
